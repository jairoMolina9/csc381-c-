...
istream& operator>>(istream& input, BigInteger& obj) {
	...
	size_t size = test.length() + 1;
	
	obj.bint = new char[size];

	strcpy(obj.bint, test.c_str());

	obj.size = size+1;
	obj.length = test.length();
...
BigInteger BigInteger::operator/(BigInteger& divisor)
{
	...
	/*
		...
		Case 2: Divide by 1
		Case 3: Equal n/n
		Case 4: Dividen < Divisor
		Case 5: Regular division
	*/

	if (divisor.length == 1 && (divisor.bint[0] == '0')) {
		cout << " cannot divide by ";
	}
	else if (divisor.length == 1 && (divisor.bint[0] == '1')) {
		return *this;
	}
		...
		for (; idx < length; idx++) {
			...
			curr_bint = stringToBigInt(curr_str.length()+1, curr_str);
		}
